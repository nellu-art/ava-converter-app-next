...
import { CURRENCIES, defaultBaseCurrency } from '../constants/currencies'

const stackSize = 17

async function runPromisesInSequence<TPromiseResult>(
  data: string[],
  parallelCount: number,
  createPromise: (currency: string) => Promise<TPromiseResult>
) {
  const result = []
...
async function getCurrencyRate({
  ...
  try {
    ...
    const lastPriceValue = await page.evaluate(
      (node: any) => node.getAttribute('data-last-price'),
      target
    )
...
export async function getCurrencyRatesFromGoogle(userCurrencies = CURRENCIES) {
  let browser: any
  let currenciesRates: { name: string; value: string }[] = []

  try {
    browser = await startBrowser()

    const results = await runPromisesInSequence(
      userCurrencies,
      stackSize,
      (currency) =>
        getCurrencyRate({
    ...
    currenciesRates = results.reduce<{ name: string; value: string }[]>(
      (res, current) =>
        current.status === 'fulfilled' ? [...res, current.value] : res,
      []
    )
  } catch (err) {
